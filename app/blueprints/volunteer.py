...
def index():
    """Volunteer console home — profile and active tasks."""
...
@require_role("volunteer")
def api_profile():
    """Return the current volunteer's profile."""
    vol = _svc.get_volunteer_by_id(DEMO_VOLUNTEER_ID)
    if not vol:
...
@require_role("volunteer")
def api_tasks():
    """Return all tasks assigned to the current volunteer."""
...
@require_role("volunteer")
def api_ai_guidance():
    """Get AI-generated guidance for a specific task."""
    body = request.get_json(silent=True) or {}
...
@require_role("volunteer")
def api_sos():
    """Submit an SOS escalation, creating a high-severity incident."""
    body = request.get_json(silent=True) or {}
